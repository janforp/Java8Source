...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
     ...
     * @param key key
     * @param value 替换陈什么
     * @param cv comparedValue 如果cv != null
     */
                            ...
                            for (; ; ) {
                                ...
                                if (e.hash == hash && ((ek = e.key) == key || (ek != null && key.equals(ek)))) {
                                    ...
                                    if (
                                        ...
                                        //将当前节点的值 赋值给 oldVal 后续返回会用到，这个不重要
                                        oldVal = ev;
                                        ...
                                        //前提：value == null
                                        //条件成立：说明当前节点非头结点，则在链表中，使用单向链表删除方法即可
                                        else if (pred != null) {
    ...
    private final void addCount(long addNum, int check) {
        //counterCellsRef 表示 LongAdder.cells
        CounterCell[] counterCellsRef;
        //asBase 表示LongAdder.base
        long asBase;
        //sizeOfTab 表示当前map.table中元素的数量
        long sizeOfTab;
        if (
            ...
            //       false->表示当前线程应该将数据累加到 base
                (counterCellsRef = counterCells) != null
                        ...
                        !U.compareAndSwapLong(
                                this,//cas字段属于当前对象本身
                                BASECOUNT,//cas字段在当前对象的内存偏移量
                                asBase = baseCount,//asBase 赋值，其实就是期望值
                                //修改之后的值，在此处进行数量的递增
            ...
            CounterCell hitCell;
            //valueOfHitCell 表示当前线程写cell时的期望值
            long valueOfHitCell;
            //TODO m 表示当前cells数组的长度，实际是 m = counterCellsRef.length - 1 ？
            int m;
            //true -> 未竞争  false->发生竞争
            ...
            if (
                ...
                // true-> 表示当前线程是通过 写base竞争失败 然后进入的if块，就需要调用fullAddCount方法去扩容 或者 重试.. LongAdder.longAccumulate
                    counterCellsRef == null || (m = counterCellsRef.length - 1) < 0
                            ||
                            //因为 || 为短路运算符，则进入该条件判断的前置条件：cells已经初始化了
                            //条件二：hitCell = counterCellsRef[ThreadLocalRandom.getProbe() & m]) == null
        ...
        //check为负数的时候，是删除操作调用的该方法，不会有扩容
        if (check >= 0) {
            //tab 表示map.table
            //tempNextTab 表示map.nextTable
            Node<K, V>[] tab;
            Node<K, V>[] tempNextTab;
            ...
            //自旋
            while (
                //条件一：sizeOfTab >= (long)(sc = sizeCtl)
                //       true-> 1.当前sizeCtl为一个负数 表示正在扩容中..
                //              2.当前sizeCtl是一个正数，表示扩容阈值
                //       false-> 表示当前table尚未达到扩容条件
                    sizeOfTab >= (long) (sc = sizeCtl)//看看是否达到扩容的阈值或者当前已经在扩容了，当前线程需要帮忙扩容
                            &&
                            //条件二：(tab = table) != null
                            //       恒成立 true
                            (tab = table) != null//只有数组已经初始化了之后才有扩容的概念
                            &&
                ...
                int currentResizeStamp = resizeStamp(n);
                ...
                if (sc < 0) {
                    //表示当前正在扩容，当前线程需要帮忙扩容
                    if (
                        //条件一：(sc >>> RESIZE_STAMP_SHIFT) != currentResizeStamp 右移16位，之后不等于 currentResizeStamp，说明扩容批次晚了，则当前线程不需要扩容了
                        //      true->说明当前线程获取到的扩容唯一标识戳 非 本批次扩容，可能他的批次位 16 到 32的，但是目前已经在执行 32 到 64 的扩容了，明显当前线程就不要扩容了
                        //      false->说明当前线程获取到的扩容唯一标识戳 是 本批次扩容
                            (sc >>> RESIZE_STAMP_SHIFT) != currentResizeStamp
                                    ||
                                    //条件二： JDK1.8 中有bug jira已经提出来了 其实想表达的是 ：  sc == (currentResizeStamp << 16 ) + 1
                                    //        true-> 表示扩容完毕，当前线程不需要再参与进来了
                                    //        false->扩容还在进行中，当前线程可以参与
                                    sc == currentResizeStamp + 1//其实想表达的是：当前批次的扩容是否已经完成
                                    ||
                                    //MAX_RESIZERS = 65535
                                    //条件三：JDK1.8 中有bug jira已经提出来了 其实想表达的是 ： sc == (currentResizeStamp<<16) + MAX_RESIZERS
                                    //        true-> 表示当前参与并发扩容的线程达到了最大值 65535 - 1
                                    //        false->表示当前线程可以参与进来
                                    sc == currentResizeStamp + MAX_RESIZERS //其实想表达的是：当前参与扩容的线程是否达到最大值，是，则当前线程不需要参与了
                                    ||
                                    //条件四：(tempNextTab = nextTable) == null
                                    //        true->表示本次扩容结束
                                    //        false->扩容正在进行中
                                    (tempNextTab = nextTable) == null
                                    ||
                                    transferIndex <= 0) {
                        break;
                    }
                    ...
                    //条件成立：说明当前线程成功参与到扩容任务中，并且将sc低16位值加1，表示多了一个线程参与扩容工作
                    ...
                    if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1)) {
                        ...
                        transfer(tab, tempNextTab);
                    }
                }

                //如果sc是大于0的，则当前线程是触发扩容的线程
                ...
                //条件成立，说明当前线程是触发扩容的第一个线程，在transfer方法需要做一些扩容准备工作
                        ...
                        sc,
                        //左移16，然后 + 2
                        (currentResizeStamp << RESIZE_STAMP_SHIFT) + 2)) {
    ...
    final Node<K, V>[] helpTransfer(Node<K, V>[] tab, Node<K, V> f) {
        ...
        if (
            ...
            //拿当前表的长度 获取 扩容标识戳   假设 16 -> 32 扩容：1000 0000 0001 1011
            ...
            while (
                ...
                //       2.再次触发扩容了...咱们拿到的nextTab 已经是上一次扩容的时候生成的 nextTable了，已经过期了...
                    nextTab == nextTable
                            &&
                            //条件二：table == tab
                            //成立：说明 扩容正在进行中，还未完成
                            //不成立：说明扩容已经结束了，扩容结束之后，最后退出的线程 会设置 table = nextTable（table引用发生变化了）
                            table == tab
                            &&
                            //条件三：(sc = sizeCtl) < 0
                            //成立：说明扩容正在进行中
                ...
                if (
                    ...
                    //      false->说明当前线程获取到的扩容唯一标识戳 是 本批次扩容
                        (sc >>> RESIZE_STAMP_SHIFT) != rs//判断扩容批次是否相同
                                ||
                                ...
                                //        true-> 表示当前参与并发扩容的线程达到了最大值 65535 - 1
                                //        false->表示当前线程可以参与进来
                                sc == rs + MAX_RESIZERS//判断参与扩容的线程是否到达最大值
                                ||
                                //条件四：transferIndex <= 0
                                //      true->说明map对象全局范围内的任务已经分配完了，当前线程进去也没活干..
                                //      false->还有任务可以分配。
                                transferIndex <= 0) {//判断是否还有扩容任务能够分配给当前线程

                    //就不要去帮忙扩容了
                    break;
                }

                //到这里就说明，当前扩容还没有完成，仍然有机会去帮忙扩容，加油

                //更新sc的低16位，参与扩容的线程数量 + 1
                if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1)) {
                    transfer(tab, nextTab);
                    break;
                }
            }
            //扩容完成之后，nextTab 其实就是 table
            return nextTab;
        }
        //老表
        return table;
        ...
        for (; ; ) {
            ...
            while (advance) {
                ...
                //条件不成立：表示对象全局范围内的桶位尚未分配完毕，还有区间可分配
                    ...
                    //transferIndex <= 0 说明迁移工作全部分配完成，当前线程没有任务
                    i = -1;
                    advance = false;
                }
                //CASE3:
                ...
                //条件成立：说明给当前线程分配任务成功
                //条件失败：说明分配给当前线程失败，应该是和其它线程发生了竞争吧，继续while循环
                else if (U.compareAndSwapInt(
                        this,
                        ...
                        //最终赋值给了下界
                        //这个三目运算：完成了nextIndex的赋值，并且完成了全局的transferIndex赋值
                        nextBound = (//分配任务区间的逻辑
            ...
            if (
                //上面的while循环的分支 (nextIndex = transferIndex) <= 0 ，
                //此时说明任务已经分配完成，当前线程没有任务,i = -1;
                    i < 0
                            ||
                            //不会成立，因为 i = nextIndex - 1;nextIndex 最大为n
                            i >= n
                            ||
                            //不会成立
                            //nextNewTabLen = 2 * n
                ...
                //当前线程没有任务，退出
                //因为进来的时候 当前线程把 sizeCtl 的后16位加了1，退出的时候当然要减1
                ...
                if (U.compareAndSwapInt(this, SIZECTL, sc = sizeCtl, sc - 1)) {
                    ...
                    if ((sc - 2) != resizeStamp(n) << RESIZE_STAMP_SHIFT) {
                        //条件成立：当前线程不是最后一个退出扩容的线程
                        //正常退出
                        return;
                    }

                    //如果 (sc - 2) == resizeStamp(n) << RESIZE_STAMP_SHIFT
                    //说明当前线程是最后一个，则需要加班，把 finsh 设置位结束了
    ...
    static final class TreeBin<K, V> extends Node<K, V> {
         ...
         * 3.等待者状态（写线程在等待），当TreeBin中有读线程目前正在读取数据时，
         * 写线程无法修改数据，那么就将lockState的最低2位 设置为 0b 10
         *
         * 二进制的 10 表示：0b 10, 等于十进制的 2
         */
        ...
        TreeBin(TreeNode<K, V> b) {
            ...
            for (
                ...
                if (r == null) {
                    ...
                    for (; ; ) {
                        ...
                        //当前要插入节点的hash跟当前遍历到的节点的hash相同
                        //如果执行到 CASE3，说明当前插入节点的hash 与 当前节点的hash一致，会在case3 做出最终排序。最终
                        //拿到的dir 一定不是0，而是（-1， 1）中的一个
                        else if (
                            //如果k拿到的kc为null,则去决战方法 tieBreakOrder
                                (kc == null && (kc = comparableClassFor(k)) == null)
                                        ||
                                        //如果kc不为null则继续在此条件执行,如果对比的值还是一样，则去决战方法
                                        (dir = compareComparables(kc, k, pk)) == 0) {
                        ...
                        if (
                            //通过 dir 来给 p 赋值
                                (p = (dir <= 0)
                                        ...
                                        //有孩子
                                        p.right)
                                        //如果 p 为 null,如果不为 null 则到p的子节点中继续循环搜索
                                        == null
                        ) {
                            //设置插入节点的父节点 为 当前节点
                            x.parent = xp;
            ...
            //将平衡之后的红黑树的跟节点 赋值给 TreeBin对象的 root引用。
            this.root = r;
         ...
         *
         * 返回匹配的节点，如果没有则返回null。尝试从根目录使用树比较进行搜索，但是在锁不可用时继续线性搜索。
         */
                ...
                for (Node<K, V> e = first; e != null; ) {
                    ...
                    if ((
                            (s = lockState) &
                                    ...
                                    (WAITER | WRITER)
                            //TODO lockState & 0011 != 0 ,说明当前TreeBin 有等待者线程 或者 目前有写操作线程正在加锁？？？？？？？
                    ) != 0) {
                        //表示当前红黑树需要阻塞，所以，为了避免阻塞，使用链表查询
                        if (e.hash == h && ((ek = e.key) == k || (ek != null && k.equals(ek)))) {
                            //如果使用链表查询到了，则返回，这样避免来阻塞
                            return e;
                        }
                        //如果当前节点的key不是要查询的，则查询链表的下一个节点
                        e = e.next;
                    }

                    //前置条件：当前TreeBin中 没有等待者线程 并且 没有写线程，也就是没有独占锁!!!
                    else if (
                        //如果cas成功：说明添加读锁成功
                        ...
                        try {
                            ...
                            p = (
                                    //r赋值跟节点，r是否为null?
                                    (r = root) == null ?
                                            //如果跟节点为null,则当前查询节点为null
                                            null
                                            :
                                            //如果跟节点不为null，则去该红黑树中查询到目标节点，当然也可能查不到
                            ...
                            if (
                                ...
                                //并且返回 减去4之前的值
                                    U.getAndAddInt(this, LOCKSTATE, -READER)
                                            //lockState 的值减去4之前等于 (READER | WAITER)，说明当前读线程为 TreeBin中的最后一个读线程。
